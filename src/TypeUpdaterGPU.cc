 ...
 */
TypeUpdaterGPU::TypeUpdaterGPU(std::shared_ptr<SystemDefinition> sysdef) : TypeUpdater(sysdef)
    {
...
void TypeUpdaterGPU::changeTypes(unsigned int timestep)
    {
    if (m_prof)
        m_prof->push(m_exec_conf, "type update");
    ...
    m_tuner->begin();
    gpu::change_types_region(d_pos.data,
                             m_inside_type,
                             m_outside_type,
                             m_z_lo,
                             m_z_hi,
...
void export_TypeUpdaterGPU(pybind11::module& m)
    ...
    py::class_<TypeUpdaterGPU, std::shared_ptr<TypeUpdaterGPU>>(m,
                                                                "TypeUpdaterGPU",
                                                                py::base<TypeUpdater>())
