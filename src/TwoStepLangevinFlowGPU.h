...
class PYBIND11_EXPORT TwoStepLangevinFlowGPU : public azplugins::TwoStepLangevinFlow<FlowField>
    ...
    TwoStepLangevinFlowGPU(std::shared_ptr<SystemDefinition> sysdef,
                           ...
                           Scalar lambda,
                           bool noiseless)
        : azplugins::TwoStepLangevinFlow<FlowField>(sysdef,
                                                    group,
                                                    T,
                                                    flow_field,
                                                    seed,
                                                    ...
                                                    noiseless)
        {
        this->m_tuner_1.reset(
            new Autotuner(32, 1024, 32, 5, 100000, "langevin_flow_1", this->m_exec_conf));
        this->m_tuner_2.reset(
            new Autotuner(32, 1024, 32, 5, 100000, "langevin_flow_2", this->m_exec_conf));
        }
    ...
    virtual void setAutotunerParams(bool enable, unsigned int period)
        ...
        this->m_tuner_1->setPeriod(period);
        this->m_tuner_1->setEnabled(enable);
        this->m_tuner_2->setPeriod(period);
        this->m_tuner_2->setEnabled(enable);
        }
...
void TwoStepLangevinFlowGPU<FlowField>::integrateStepOne(unsigned int timestep)
    ...
    if (this->m_aniso)
        {
        this->m_exec_conf->msg->error() << "azplugins.integrate: anisotropic particles are not "
                                           "supported with langevin flow integrators."
                                        << std::endl;
        throw std::runtime_error("Anisotropic integration not supported with langevin flow");
        }
    if (this->m_prof)
        this->m_prof->push(this->m_exec_conf, "Langevin step 1");
...
void TwoStepLangevinFlowGPU<FlowField>::integrateStepTwo(unsigned int timestep)
    ...
    if (this->m_prof)
        this->m_prof->push(this->m_exec_conf, "Langevin step 2");
    ...
    // this template could be implicitly deduced, but writing it explicitly to make it clear to the
    // caller
...
void export_TwoStepLangevinFlowGPU(pybind11::module& m, const std::string& name)
    ...
    py::class_<LangevinFlowGPU, std::shared_ptr<LangevinFlowGPU>>(m,
                                                                  name.c_str(),
                                                                  py::base<LangevinFlow>())
