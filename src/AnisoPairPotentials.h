 ...
 * AnisoPotentialPair, which uses an evaluator functor to compute the actual details of the pair
 ...
 * AnisoPairEvaluatorMyGreatPotential.h. This file should be included below. You can follow one of
 * the other evaluator functors as an example for the details.
 *
 *  2. Explicitly instantiate a template for a CUDA driver for your potential in
 * AnisoPairPotentials.cu.
 *
 *  3. Expose the anisotropic pair potential on the python level in module.cc using
 * export_aniso_pair_potential and add the mirror python object to pair.py.
 *
...
template<class evaluator> void export_aniso_pair_potential(py::module& m, const std::string& name)
    {
    typedef ::AnisoPotentialPair<evaluator> pair_potential_cpu;
    export_AnisoPotentialPair<pair_potential_cpu>(m, name);

#ifdef ENABLE_CUDA
    typedef ::AnisoPotentialPairGPU<evaluator,
                                    azplugins::gpu::compute_aniso_pair_potential<evaluator>>
        pair_potential_gpu;
