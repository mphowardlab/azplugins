...
GroupVelocityCompute::GroupVelocityCompute(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : Compute(sysdef), m_group(group), m_lognames {"vx" + suffix, "vy" + suffix, "vz" + suffix},
...
Scalar GroupVelocityCompute::getLogValue(const std::string& quantity, uint64_t timestep)
    ...
    if (quantity == m_lognames[0])
        ...
        {
        m_exec_conf->msg->error() << "GroupVelocityCompute: " << quantity
...
void export_GroupVelocityCompute(pybind11::module& m)
    ...
    py::class_<GroupVelocityCompute, std::shared_ptr<GroupVelocityCompute>>(m,
