...
TypeUpdater::TypeUpdater(std::shared_ptr<SystemDefinition> sysdef)
    : Updater(sysdef), m_inside_type(0xffffffff), m_outside_type(0xffffffff), m_z_lo(1.0),
      m_z_hi(-1.0), m_check_types(true), m_check_region(true)
    {
    m_exec_conf->msg->notice(5) << "Constructing TypeUpdater" << std::endl;

    // subscribe to number of type signal, in case number of types is allowed to decrease in future
    // versions of hoomd
...
TypeUpdater::TypeUpdater(std::shared_ptr<SystemDefinition> sysdef,
                         ...
                         Scalar z_lo,
                         Scalar z_hi)
    : Updater(sysdef), m_inside_type(inside_type), m_outside_type(outside_type), m_z_lo(z_lo),
      m_z_hi(z_hi), m_check_types(true), m_check_region(true)
    {
    m_exec_conf->msg->notice(5) << "Constructing TypeUpdater" << std::endl;
...
TypeUpdater::~TypeUpdater()
    ...
    m_pdata->getNumTypesChangeSignal().disconnect<TypeUpdater, &TypeUpdater::requestCheckTypes>(
        this);
...
void TypeUpdater::checkTypes() const
    ...
    if (m_inside_type == m_outside_type)
        ...
        m_exec_conf->msg->error() << "TypeUpdater: inside and outside type (" << name
                                  << ") cannot match." << std::endl;
...
void TypeUpdater::checkRegion() const
    ...
    if (m_z_lo >= m_z_hi)
        {
        m_exec_conf->msg->error() << "TypeUpdater: lower z bound " << m_z_lo << " >= upper z bound "
                                  << m_z_hi << "." << std::endl;
    ...
    if (m_z_lo < global_lo.z)
        ...
        m_exec_conf->msg->error() << "TypeUpdater: lower z bound " << m_z_lo
                                  << " lies outside simulation box lower bound " << global_lo.z
                                  << "." << std::endl;
    ...
    if (m_z_hi > global_hi.z)
        ...
        m_exec_conf->msg->error() << "TypeUpdater: upper z bound " << m_z_hi
                                  << " lies outside simulation box upper bound " << global_hi.z
                                  << "." << std::endl;
