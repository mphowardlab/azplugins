...
def snap():
    ...
    if snap_.communicator.rank == 0:
        ...
        snap_.particles.position[:] = [[0, 0, 4.6], [0, 0, -5.4], [0, 5.6, 0], [6.6, 0, 0]]
...
@pytest.mark.parametrize("cls", [
    external.SphericalHarmonicBarrier
], ids=["Spherical"])

def test_spherical_harmonic_barrier(simulation_factory, cls, integrator, snap, custom_variant_fixture):
...
@pytest.mark.parametrize("cls", [
    external.PlanarHarmonicBarrier
], ids=["Planar"])

def test_planar_harmonic_barrier(simulation_factory, cls, integrator, custom_variant_fixture):
    snap = hoomd.Snapshot()
    if snap.communicator.rank == 0:
        snap.configuration.box = [20, 20, 20, 0, 0, 0]
        snap.particles.N = 4
        snap.particles.types = ['A', 'B']
        snap.particles.position[:] = [[1, 1, 4.6], [-1, 1, 5.4], [1, -1, 5.6], [-1, -1, 6.6]]
