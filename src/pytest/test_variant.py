...
def interpolated_eval(data, t_lo, t_hi):
    ...
    return hoomd.azplugins.variant.VariantInterpolated(
        data=data, t_lo=t_lo, t_hi=t_hi
    )
...
@pytest.mark.parametrize("data, t_lo, t_hi", variant_cases)
def test_construction(data, t_lo, t_hi):
    """t_lo and t_hi are stored and exposed as read-only properties."""
...
@pytest.mark.parametrize("data, t_lo, t_hi", variant_cases)
def test_interpolated_values(data, t_lo, t_hi):
...
@pytest.mark.parametrize("data, t_lo, t_hi", variant_cases)
def test_boundary_values(data, t_lo, t_hi):
    """At t_lo the variant returns data[0]; at t_hi it returns data[-1]."""
    variant = interpolated_eval(data, t_lo, t_hi)
    assert variant(int(t_lo)) == pytest.approx(data[0])
    assert variant(int(t_hi)) == pytest.approx(data[-1])
 
 
@pytest.mark.parametrize("data, t_lo, t_hi", variant_cases)
def test_min_max(data, t_lo, t_hi):
    """min and max report the extrema of the data array."""
    variant = interpolated_eval(data, t_lo, t_hi)
