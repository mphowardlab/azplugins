...
ImplicitEvaporator::ImplicitEvaporator(std::shared_ptr<SystemDefinition> sysdef,
    ...
    m_pdata->getNumTypesChangeSignal()
        .connect<ImplicitEvaporator, &ImplicitEvaporator::reallocateParams>(this);
    }

ImplicitEvaporator::~ImplicitEvaporator()
    {
    m_pdata->getNumTypesChangeSignal()
        .disconnect<ImplicitEvaporator, &ImplicitEvaporator::reallocateParams>(this);
    }
...
void ImplicitEvaporator::computeForces(unsigned int timestep)
    ...
    if (!m_has_warned
        ...
        {
        m_exec_conf->msg->warning() << "ImplicitEvaporator does not compute its virial "
...
void export_ImplicitEvaporator(pybind11::module& m)
    ...
    py::class_<ImplicitEvaporator, std::shared_ptr<ImplicitEvaporator>>(m,
                                                                        "ImplicitEvaporator",
