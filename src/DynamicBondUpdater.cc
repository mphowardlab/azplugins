...
DynamicBondUpdater::DynamicBondUpdater(std::shared_ptr<SystemDefinition> sysdef,
                                       ...
                                       uint16_t seed)
    : Updater(sysdef, trigger), m_group_1(group_1), m_group_2(group_2), m_groups_identical(false),
      m_r_cut(0), m_probability(0.0), m_bond_type(0), m_max_bonds_group_1(0),
      m_max_bonds_group_2(0), m_seed(seed), m_pair_nlist(pair_nlist),
    ...
    m_pdata->getGlobalParticleNumberChangeSignal()
        .connect<DynamicBondUpdater, &DynamicBondUpdater::slotNumParticlesChanged>(this);

    m_bond_data = m_sysdef->getBondData();

    m_pair_internal_nlist
        = std::shared_ptr<hoomd::md::NeighborList>(new hoomd::md::NeighborListTree(sysdef, 0.0));
...
DynamicBondUpdater::DynamicBondUpdater(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : Updater(sysdef, trigger), m_group_1(group_1), m_group_2(group_2), m_groups_identical(false),
      ...
      m_max_bonds_group_1(max_bonds_group_1), m_max_bonds_group_2(max_bonds_group_2), m_seed(seed),
...
DynamicBondUpdater::~DynamicBondUpdater()
    ...
    m_pdata->getBoxChangeSignal()
        .disconnect<DynamicBondUpdater, &DynamicBondUpdater::slotBoxChanged>(this);
    m_pdata->getGlobalParticleNumberChangeSignal()
        .disconnect<DynamicBondUpdater, &DynamicBondUpdater::slotNumParticlesChanged>(this);
    }
...
void DynamicBondUpdater::update(uint64_t timestep)
    ...
    do
        ...
        ArrayHandle<unsigned int> h_n_neigh(m_pair_internal_nlist->getNNeighArray(),
...
// sort according to that, but there might be the case where multiple possible bond lengths are
// exactly identical, e.g. particles on a lattice. This is hiracical sorting: first according to
// possible bond distance r_ab_sq, then after first tag_a, last after second tag_b. Should work
// given that the tags are oredered within each pair, (tag_a,tag_b,d_ab_sq) with tag_a < tag_b.

// todo: because it's possible uniquely map a pair to a single int (and back!) with a pairing
// function, the possible bond array could be restructured into a different data structure? if we
// don't keep the possible bond length, a unsigned int array could hold all information needed when
// would that lead to problems with overflow from 0.5*(tag_a+tag_b)*(tag_a+tag_b+1)+tag_b being too
// large?
...
bool DynamicBondUpdater::CheckisExistingLegalBond(Scalar3 i)
    ...
    // (0,0,0.0) is the default "empty" value - todo: have a "invalid" unsigned int ? how to
    // fill/reset with memset()?
    if (tag_1 == 0 && tag_2 == 0)
        {
        return true;
        }
...
void DynamicBondUpdater::calculateExistingBonds()
    {
        ...
        ArrayHandle<unsigned int> h_existing_bonds_list(m_existing_bonds_list,
                                                        access_location::host,
                                                        access_mode::overwrite);
        memset((void*)h_existing_bonds_list.data,
               0,
               sizeof(unsigned int) * m_group_1->getNumMembers()
                   * m_existing_bonds_list_indexer.getH());
        }
        {
...
void DynamicBondUpdater::resizeExistingBondList()
    {
        ...
        m_existing_bonds_list_indexer
            = Index2D((unsigned int)m_existing_bonds_list.getPitch(), new_height);
        m_exec_conf->msg->notice(6) << "DynamicBondUpdater: (Re-)size existing bond list, new size "
                                    << new_height << " bonds per particle " << std::endl;
        }
    }
...
void DynamicBondUpdater::resizePossibleBondlists()
    {
        ...
        m_exec_conf->msg->notice(6) << "DynamicBondUpdater: (Re-)size possible bond list, new size "
                                    << m_max_bonds << " bonds per particle " << std::endl;
        }
    }
...
void DynamicBondUpdater::allocateParticleArrays()
    {
        ...
        m_existing_bonds_list_indexer = Index2D((unsigned int)m_existing_bonds_list.getPitch(),
                                                (unsigned int)m_existing_bonds_list.getHeight());
        ...
        memset(h_n_existing_bonds.data,
               0,
               sizeof(unsigned int) * m_n_existing_bonds.getNumElements());
        memset(h_existing_bonds_list.data,
               0,
               sizeof(unsigned int) * m_existing_bonds_list.getNumElements());
        }
    calculateExistingBonds();
    }

/*! This function takes the information about neighbors between group_2 and group_1 saved in m_nlist
 * and m_n_neigh and copies pairs within the m_r_cut cutoff distance into the m_all_possible_bonds
 * array. Then, all invalid (0,0,0), dublicated, and existing bonds  are removed from
 * m_all_possible_bonds. It is sorted by distance  (shortest to longest) between the two particles
 * in the possible bond.
 */
void DynamicBondUpdater::filterPossibleBonds()
    {
        // copy data from h_n_list to h_all_possible_bonds
        {
        ArrayHandle<unsigned int> h_nlist(m_pair_internal_nlist->getNListArray(),
                                          access_location::host,
                                          access_mode::read);
        ArrayHandle<unsigned int> h_n_neigh(m_pair_internal_nlist->getNNeighArray(),
                                            access_location::host,
                                            access_mode::read);
        ArrayHandle<size_t> h_n_head_list(m_pair_internal_nlist->getHeadList(),
                                          access_location::host,
                                          access_mode::read);
        ...
        ArrayHandle<Scalar3> h_all_possible_bonds(m_all_possible_bonds,
                                                  access_location::host,
                                                  access_mode::readwrite);
        unsigned int size = m_group_1->getNumMembers() * m_max_bonds;
                    ...
                    if (dr_sq < r_cutsq)
                        ...
                        if (n_curr_bond < m_max_bonds)
                            ...
                            if (m_groups_identical)
                                ...
                                d = make_scalar3(__int_as_scalar(tag_a),
                                                 __int_as_scalar(tag_b),
                                                 dr_sq);
                                }
                            else
                                {
                                d = make_scalar3(__int_as_scalar(tag_i),
                                                 __int_as_scalar(tag_j),
                                                 dr_sq);
                                }
        ...
        std::sort(h_all_possible_bonds.data,
        ...
        auto last = std::unique(h_all_possible_bonds.data,
                                h_all_possible_bonds.data + m_num_all_possible_bonds,
                                CompareBonds);
 ...
 *  should be updated.
 *
 * Note: this function is very hard to parallelize on the GPU since we need to go through the bonds
 * sequentially to prevent forming too many bonds in one step. Have not found a good way of doing
 * this on the GPU.
 */
void DynamicBondUpdater::makeBonds(uint64_t timestep)
    ...
    // so that we don't end up forming too many bonds in one step. "AddtoExistingBonds" increases
    // the count in h_n_existing_bonds in the for loop below as we go, so no extra bookkeeping
    // should be needed. This also makes it very difficult to do on the GPU.
    ...
    for (unsigned int i = 0; i < m_num_all_possible_bonds; i++)
        ...
        // todo: put in other external criteria here, e.g. max number of bonds possible in one step,
        // etc. todo: randomize which bonds are formed or keep them ordered by their distances ?
        ...
        if ((m_max_bonds_group_1 > h_n_existing_bonds.data[tag_i])
            && (m_max_bonds_group_2 > h_n_existing_bonds.data[tag_j]) && (random < m_probability))
            {
...
void DynamicBondUpdater::checkBoxSize()
    ...
    if ((periodic.x && nearest_plane_distance.x <= rmax * 2.0)
        ...
        {
        m_exec_conf->msg->error() << "DynamicBondUpdater: Simulation box is too small! Particles "
                                     "would be interacting with themselves."
                                  << std::endl;
...
void DynamicBondUpdater::setGroupOverlap()
    ...
    if (m_group_1->getNumMembers() == 0)
        {
        m_exec_conf->msg->warning() << "DynamicBondUpdater: First group group_1 appears to be "
                                       "empty. No bonds will be formed. "
                                    << std::endl;
        }

    if (m_group_2->getNumMembers() == 0)
        {
        m_exec_conf->msg->warning() << "DynamicBondUpdater: Second group group_2 appears to be "
                                       "empty. No bonds will be formed. "
                                    << std::endl;
        }
        {
        // check if the two groups are either identical or have no overlap
        ArrayHandle<unsigned int> h_index_group_1(m_group_1->getIndexArray(),
                                                  access_location::host,
                                                  access_mode::read);
        ...
        if (overlap > 0 && overlap != m_group_1->getNumMembers())
            ...
            m_exec_conf->msg->error()
                << "DynamicBondUpdater: group 1 and group 2 have " << overlap
                << " overlaps. Partially overlapping groups are not implemented." << std::endl;
...
void DynamicBondUpdater::setCutoffs()
    {
        {
        // set all rcuts to zero first, then only set the one between group1 and group 2 particle
        // types to be m_r_cut
        ...
        if (m_groups_identical == false)
            {
            ArrayHandle<unsigned int> h_index_group_2(m_group_2->getIndexArray(),
                                                      access_location::host,
                                                      access_mode::read);
...
void DynamicBondUpdater::checkRcut()
    ...
    if (m_r_cut <= 0.0)
        {
        m_exec_conf->msg->error()
            << "DynamicBondUpdater: Requested cutoff distance is less than or equal to zero"
            << std::endl;
...
void DynamicBondUpdater::checkProbability()
    ...
    if (m_probability < 0.0)
        {
        m_exec_conf->msg->error() << "DynamicBondUpdater: Requested probability  is less than zero"
                                  << std::endl;
        throw std::runtime_error("Error initializing DynamicBondUpdater");
        }
    else if (m_probability > 1.0)
        {
        m_exec_conf->msg->error() << "DynamicBondUpdater: Requested probability is larger than one"
                                  << std::endl;
        throw std::runtime_error("Error initializing DynamicBondUpdater");
        }
    }
...
void DynamicBondUpdater::checkMaxBondsGroup()
    ...
    if (m_max_bonds_group_1 < 0.0 or m_max_bonds_group_2 < 0.0)
        {
        m_exec_conf->msg->error() << "DynamicBondUpdater: Max number of bonds that groups can form "
                                     "is negative. Check parameters."
                                  << std::endl;
        throw std::runtime_error("Error initializing DynamicBondUpdater");
        }
    else if (m_max_bonds_group_1 > 10 or m_max_bonds_group_2 > 10)
        {
        m_exec_conf->msg->warning() << "DynamicBondUpdater: Requested number of bonds that can "
                                       "form is very large. This can lead to performance issues."
                                    << std::endl;
        }
    }
...
void export_DynamicBondUpdater(pybind11::module& m)
    ...
    pybind11::class_<DynamicBondUpdater, Updater, std::shared_ptr<DynamicBondUpdater>>(
        ...
        .def_property("probability",
                      &DynamicBondUpdater::getProbability,
                      &DynamicBondUpdater::setProbability)
        .def_property("bond_type",
                      &DynamicBondUpdater::getBondType,
                      &DynamicBondUpdater::setBondType)
        .def_property("max_bonds_group_1",
                      &DynamicBondUpdater::getMaxBondsGroup1,
                      &DynamicBondUpdater::setMaxBondsGroup1)
        .def_property("max_bonds_group_2",
                      &DynamicBondUpdater::getMaxBondsGroup2,
                      &DynamicBondUpdater::setMaxBondsGroup2);
    }
