...
DynamicBondUpdater::DynamicBondUpdater(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : Updater(sysdef, trigger), m_group_1(group_1), m_group_2(group_2), m_groups_identical(false),
      ...
      m_max_bonds_group_2(0), m_seed(seed), m_pair_nlist_exclusions_set(false), m_box_changed(true),
    ...
    if (m_exec_conf->getNRanks() > 1)
        {
        m_exec_conf->msg->error() << "DynamicBondUpdater: MPI not implemented." << std::endl;
        throw std::runtime_error("Error in DynamicBondUpdater, MPI not implemented.");
        }
...
DynamicBondUpdater::DynamicBondUpdater(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : Updater(sysdef, trigger), m_group_1(group_1), m_group_2(group_2), m_groups_identical(false),
      ...
      m_pair_nlist_exclusions_set(false), m_box_changed(true), m_max_N_changed(true)
    {
    m_exec_conf->msg->notice(5) << "Constructing DynamicBondUpdater" << std::endl;
...
void DynamicBondUpdater::update(uint64_t timestep)
    ...
    if (m_max_N_changed)
        ...
        // if more neighbors than m_max_bonds, resize the list that saves the possible bonds for all
        // particles
...
// sort according to that, but there might be the case where multiple possible bond lengths
// are exactly identical, e.g. particles on a lattice.
// This is hiracical sorting: first according to possible bond distance r_ab_sq, then after
// first tag_a, last after second tag_b.
// Should work given that the tags are oredered within each pair, (tag_a,tag_b,d_ab_sq)
// with tag_a < tag_b.
...
void DynamicBondUpdater::calculateExistingBonds()
    ...
    for (unsigned int i = 0; i < size; i++)
        ...
        if (overflowed)
            ...
            { // explicit scoping such that resizeExistingBondList can resize the
              // m_existing_bonds_list array before this
...
void DynamicBondUpdater::makeBonds(uint64_t timestep)
    ...
    for (unsigned int i = 0; i < m_num_all_possible_bonds; i++)
        ...
        if ((m_max_bonds_group_1 > h_n_existing_bonds.data[tag_i])
            ...
            // the internal neigh list should always get the exclusions updates to not re-find
            // already existing bonds over and over
