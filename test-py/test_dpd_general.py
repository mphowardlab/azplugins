...
class dpd_general_tests(unittest.TestCase):
    ...
    def test_set_params(self):
        ...
        dpd.set_params(kT=hoomd.variant.linear_interp([[0,1.],[100,2.]]))
...
class dpd_general_validation_tests(unittest.TestCase):
    ...
    def test_dissipative(self):
        snap = hoomd.data.make_snapshot(N=1000, box=hoomd.data.boxdim(L=6),particle_types=['A'])
        ...
        logger = hoomd.analyze.log(filename=None, quantities=['temperature'], period=1)
        kT = []
        cb = lambda timestep : kT.append(logger.query('temperature'))
        hoomd.analyze.callback(callback=cb, period=1)
        hoomd.run(100)
