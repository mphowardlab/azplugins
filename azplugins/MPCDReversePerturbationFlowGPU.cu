 ...
 * \brief Definition of kernel drivers and kernels for MPCDReversePerturbationFlowGPU
 */
 ...
 */
__global__ void mpcd_mark_particles_in_slabs(Scalar2 *d_slab_pairs,
                                             const Scalar4 *d_pos,
                                             const Scalar4 *d_vel,
                                             const Scalar mass,
    ...
    // tag needs to shifted by 1 so that particle 0 can be treated correctly
    int shifted_tag = int(sign)*(int(idx)+1);
    d_slab_pairs[idx] = make_scalar2(__int_as_scalar(shifted_tag), momentum);
    }
 ...
 *
 */
__global__ void mpcd_find_split_array(unsigned int *d_split,
                                      int *d_type,
 ...
 * are sorted according to their momentum, the particle with the
 * momentum closest to the target value in +x-direction in the bottom slab swaps
 * momentum with the particle with the momentum closest to the target
 * momentum in -x-direction in the top slab.  In total, num_pairs swaps are performed.
 */
__global__ void mpcd_swap_momentum_pairs(const Scalar2 *d_layer_hi,
    ...
    // swap velocities
    d_vel[lo_idx].x =  hi_momentum / mass;
    d_vel[hi_idx].x =  lo_momentum / mass;
    }
 ...
 * according to the momentum value closest to the target value.
 */
__global__ void mpcd_divide_pair_array(const Scalar2 *d_slab_pairs,
                                       Scalar2 *d_layer_hi,
                                       Scalar2 *d_layer_lo,
...
cudaError_t mpcd_mark_particles_in_slabs(Scalar2 *d_slab_pairs,
    ...
    if (max_block_size == UINT_MAX)
        ...
        cudaFuncGetAttributes(&attr, (const void*)kernel::mpcd_mark_particles_in_slabs);
        max_block_size = attr.maxThreadsPerBlock;
        }
    const int run_block_size = min(block_size, max_block_size);

    kernel::mpcd_mark_particles_in_slabs<<<N/run_block_size + 1, run_block_size>>>(d_slab_pairs,
                                                                                   d_pos,
                                                                                   d_vel,
                                                                                   mass,
                                                                                   d_lo_pos,
 ...
 * \param p_target  target velocity for the pairs to swap
...
cudaError_t mpcd_divide_pair_array(const Scalar2 *d_slab_pairs,
    ...
    if (max_block_size == UINT_MAX)
        {
        cudaFuncAttributes attr;
        cudaFuncGetAttributes(&attr, (const void*)kernel::mpcd_divide_pair_array);
        max_block_size = attr.maxThreadsPerBlock;
        }

    const int run_block_size = min(block_size, max_block_size);
    kernel::mpcd_divide_pair_array<<<num_threads/run_block_size+1, run_block_size>>>(d_slab_pairs,
                                                                                     d_layer_hi,
                                                                                     d_layer_lo,
                                                                                     num_hi_entries,
                                                                                     Nslab,
...
cudaError_t mpcd_find_split_array(unsigned int *d_split,
    ...
    if (max_block_size == UINT_MAX)
        {
        cudaFuncAttributes attr;
        cudaFuncGetAttributes(&attr, (const void*)kernel::mpcd_find_split_array);
        max_block_size = attr.maxThreadsPerBlock;
        }
    const int run_block_size = min(block_size, max_block_size);
    kernel::mpcd_find_split_array<<<(Nslab-1)/run_block_size+1, run_block_size>>>(d_split,
                                                                                  d_type,
                                                                                  d_slab_pairs,
                                                                                  Nslab-1);
    return cudaSuccess;
...
cudaError_t mpcd_swap_momentum_pairs(const Scalar2 *d_layer_hi,
    ...
    if (max_block_size == UINT_MAX)
        {
        cudaFuncAttributes attr;
        cudaFuncGetAttributes(&attr, (const void*)kernel::mpcd_swap_momentum_pairs);
        max_block_size = attr.maxThreadsPerBlock;
        }
    const int run_block_size = min(block_size, max_block_size);
    kernel::mpcd_swap_momentum_pairs<<<num_pairs/run_block_size+1, run_block_size>>>(d_layer_hi,
                                                                                     d_layer_lo,
                                                                                     d_vel,
                                                                                     mass,
                                                                                     num_pairs);
