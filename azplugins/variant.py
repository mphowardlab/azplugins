...
class sphere_area(hoomd.variant._variant):
    ...
    where :math:`\alpha` is the rate of surface-area reduction per time.
    This physics corresponds to the evaporation of a droplet into stagnant air
    under the mapping:
    .. math::

        \alpha = \frac{8 \pi D_{\rm v} m \Delta p}{\rho_{\rm l} k_{\rm B} T}

    with :math:`D_{\rm v}` the diffusivity in the vapor, *m* the mass of the
    evaporating solvent molecule, :math:`\Delta p` the difference in vapor
    pressure between the droplet and far away, and :math:`\rho_{\rm l}` the
