...
/*! \file ComputeThermoSLLOD.cc
    \brief Contains code for the ComputeThermoSLLOD class
*/
...
ComputeThermoSLLOD::ComputeThermoSLLOD(std::shared_ptr<SystemDefinition> sysdef,
    ...
    : Compute(sysdef) ,m_group(group), m_ndof(1), m_ndof_rot(0), m_logging_enabled(true), m_shear_rate(shear_rate)
    {
    m_exec_conf->msg->notice(5) << "Constructing ComputeThermoSLLOD" << std::endl;
    ...
    m_logname_list.push_back(std::string("temperature") + suffix);
    m_logname_list.push_back(std::string("translational_temperature") + suffix);
    m_logname_list.push_back(std::string("rotational_temperature") + suffix);
    m_logname_list.push_back(std::string("kinetic_energy") + suffix);
    m_logname_list.push_back(std::string("translational_kinetic_energy") + suffix);
    m_logname_list.push_back(std::string("rotational_kinetic_energy") + suffix);
    m_logname_list.push_back(std::string("potential_energy") + suffix);
    m_logname_list.push_back(std::string("ndof") + suffix);
    m_logname_list.push_back(std::string("translational_ndof") + suffix);
    m_logname_list.push_back(std::string("rotational_ndof") + suffix);
    m_logname_list.push_back(std::string("num_particles") + suffix);
    m_logname_list.push_back(std::string("pressure") + suffix);
    m_logname_list.push_back(std::string("pressure_xx") + suffix);
    m_logname_list.push_back(std::string("pressure_xy") + suffix);
    m_logname_list.push_back(std::string("pressure_xz") + suffix);
    m_logname_list.push_back(std::string("pressure_yy") + suffix);
    m_logname_list.push_back(std::string("pressure_yz") + suffix);
    m_logname_list.push_back(std::string("pressure_zz") + suffix);
...
ComputeThermoSLLOD::~ComputeThermoSLLOD()
    {
    m_exec_conf->msg->notice(5) << "Destroying ComputeThermoSLLOD" << std::endl;
    }

/*! \param ndof Number of degrees of freedom to set
*/
void ComputeThermoSLLOD::setNDOF(unsigned int ndof)
    {
    if (ndof == 0)
        {
        m_exec_conf->msg->warning() << "compute.thermo: given a group with 0 degrees of freedom." << std::endl
             << "            overriding ndof=1 to avoid divide by 0 errors" << std::endl;
        ndof = 1;
        }
...
*/
void ComputeThermoSLLOD::compute(unsigned int timestep)
    {
...
std::vector< std::string > ComputeThermoSLLOD::getProvidedLogQuantities()
    {
...
Scalar ComputeThermoSLLOD::getLogValue(const std::string& quantity, unsigned int timestep)
    {
    ...
    if (quantity == m_logname_list[0])
        ...
        {
        m_exec_conf->msg->error() << "compute.thermo: " << quantity << " is not a valid log quantity" << std::endl;
...
#ifdef ENABLE_MPI
void ComputeThermoSLLOD::reduceProperties()
    {
...
void export_ComputeThermoSLLOD(pybind11::module& m)
    {
    pybind11::class_<ComputeThermoSLLOD, std::shared_ptr<ComputeThermoSLLOD> >(m,"ComputeThermoSLLOD",pybind11::base<Compute>())
    .def(pybind11::init< std::shared_ptr<SystemDefinition>,std::shared_ptr<ParticleGroup>,Scalar,const std::string& >())
    .def("setNDOF", &ComputeThermoSLLOD::setNDOF)
    .def("setRotationalNDOF", &ComputeThermoSLLOD::setRotationalNDOF)
    .def("getTemperature", &ComputeThermoSLLOD::getTemperature)
    .def("getPressure", &ComputeThermoSLLOD::getPressure)
    .def("getKineticEnergy", &ComputeThermoSLLOD::getKineticEnergy)
    .def("getTranslationalKineticEnergy", &ComputeThermoSLLOD::getTranslationalKineticEnergy)
    .def("getRotationalKineticEnergy", &ComputeThermoSLLOD::getRotationalKineticEnergy)
    .def("getPotentialEnergy", &ComputeThermoSLLOD::getPotentialEnergy)
    .def("setLoggingEnabled", &ComputeThermoSLLOD::setLoggingEnabled)
    ;
    }
