 ...
 *     for the particle at a fixed distance from the wall.
 */
...
void export_wall_potential(py::module& m, const std::string& name)
    ...
    py::class_<typename wall_evaluator::param_type , std::shared_ptr<typename wall_evaluator::param_type> >(m, (wall_evaluator::getName()+"_params").c_str())
        .def(py::init<>())
        ...
        ;
    m.def(std::string("make_"+wall_evaluator::getName()+"_params").c_str(), &make_wall_params<evaluator>);
    }
