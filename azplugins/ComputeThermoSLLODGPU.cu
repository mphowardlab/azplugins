...
__global__ void compute_thermo_partial_sums(Scalar4 *d_scratch,
                                                Scalar4 *d_net_force,
                                                Scalar *d_net_virial,
...
__global__ void compute_pressure_tensor_partial_sums(Scalar *d_scratch,
                                                Scalar4 *d_net_force,
                                                Scalar *d_net_virial,
                                                const unsigned int virial_pitch,
                                                Scalar4 *d_velocity,
...
__global__ void compute_rotational_ke_partial_sums(Scalar *d_scratch,
...
*/
__global__ void compute_thermo_final_sums(Scalar *d_properties,
                                              Scalar4 *d_scratch,
...
*/
__global__ void compute_pressure_tensor_final_sums(Scalar *d_properties,
                                              Scalar *d_scratch,
                                              BoxDim box,
                                              unsigned int group_size,
                                              unsigned int num_partial_sums,
...
extern "C" __global__
void remove_flow_field(Scalar4 *d_vel,
                                 Scalar4 *d_pos,
                                 unsigned int *d_group_members,
                                 unsigned int work_size,
                                 Scalar shear_rate,
...
extern "C" __global__
void add_flow_field(Scalar4 *d_vel,
                                Scalar4 *d_pos,
                                unsigned int *d_group_members,
                                unsigned int work_size,
                                Scalar shear_rate,
...
cudaError_t compute_thermo_partial(Scalar *d_properties,
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        kernel::compute_thermo_partial_sums<<<grid,threads, shared_bytes>>>(args.d_scratch,
                                                                        args.d_net_force,
                                                                        args.d_net_virial,
                                                                        args.virial_pitch,
                                                                        d_vel,
        ...
        if (compute_pressure_tensor)
            ...
            // run the kernel
            kernel::compute_pressure_tensor_partial_sums<<<grid, threads, shared_bytes>>>(args.d_scratch_pressure_tensor,
                                                                                      args.d_net_force,
                                                                                      args.d_net_virial,
                                                                                      args.virial_pitch,
                                                                                      d_vel,
        ...
        if (compute_rotational_energy)
            ...
            // run the kernel
            kernel::compute_rotational_ke_partial_sums<<<grid, threads, shared_bytes>>>(args.d_scratch_rot,
                                                   args.d_orientation,
                                                   args.d_angmom,
                                                   args.d_inertia,
                                                   d_body,
...
cudaError_t compute_thermo_final(Scalar *d_properties,
    ...
    // run the kernel
    kernel::compute_thermo_final_sums<<<grid, threads, shared_bytes>>>(d_properties,
                                                                   args.d_scratch,
                                                                   args.d_scratch_rot,
                                                                   args.ndof,
                                                                   box,
    ...
    if (compute_pressure_tensor)
        ...
        // run the kernel
        kernel::compute_pressure_tensor_final_sums<<<grid, threads, shared_bytes>>>(d_properties,
...
cudaError_t remove_flow_field(Scalar4 *d_vel,
    ...
    if (max_block_size == UINT_MAX)
        ...
        cudaFuncGetAttributes(&attr, (const void *)kernel::remove_flow_field);
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        kernel::remove_flow_field<<< grid, threads >>>(d_vel, d_pos, d_group_members, nwork, shear_rate, range.first);
        }
...
cudaError_t add_flow_field(Scalar4 *d_vel,
    ...
    if (max_block_size == UINT_MAX)
        {
        cudaFuncAttributes attr;
        cudaFuncGetAttributes(&attr, (const void *)kernel::add_flow_field);
        max_block_size = attr.maxThreadsPerBlock;
        }
    ...
    for (int idev = gpu_partition.getNumActiveGPUs() - 1; idev >= 0; --idev)
        ...
        // run the kernel
        kernel::add_flow_field<<< grid, threads >>>(d_vel, d_pos, d_group_members, nwork, shear_rate, range.first);
        }
