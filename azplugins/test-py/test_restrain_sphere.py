...
class restrain_sphere_tests(unittest.TestCase):
    ...
    def test_force(self):
        ...
        np.testing.assert_array_almost_equal(f.forces[1].force, (0,-2.,0), decimal=3)
        ...
        np.testing.assert_array_almost_equal(f.forces[0].virial, (-2.,2.,2.,0,0,0), decimal=3)
        np.testing.assert_array_almost_equal(f.forces[1].virial, (0,0,0,-6.,-2.,0), decimal=3)
        np.testing.assert_array_almost_equal(f.forces[2].virial, (0,0,0,0,0,24.), decimal=3)

        # change the spring constant
        f.set_params(k=1.0)
        hoomd.run(1)
        np.testing.assert_array_almost_equal(f.forces[0].force, (1.,0,0), decimal=3)
        np.testing.assert_array_almost_equal(f.forces[1].force, (0,-1.,0), decimal=3)
        np.testing.assert_array_almost_equal(f.forces[2].force, (0,0,4.), decimal=3)
        ...
        np.testing.assert_array_almost_equal(f.forces[2].force, (0,0,6.), decimal=3)
