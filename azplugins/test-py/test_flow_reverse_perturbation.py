...
class updater_reverse_perturbation(unittest.TestCase):
    def setUp(self):
        snap = hoomd.data.make_snapshot(N=3, box=hoomd.data.boxdim(L=20), particle_types=['A'])
        snap.particles.velocity[:] = (0.1,0.1,0.1)
        self.s = hoomd.init.read_snapshot(snap)

    # tests basic creation of the updater
    def test(self):
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=1.0,Nswap=1,period=100,phase=-1,target_momentum=1)
    ...
    def test_box_change(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),Nswap=1,width=3.45,period=1,target_momentum=1)
        hoomd.update.box_resize(L=5.5, period=None)
        with self.assertRaises(RuntimeError):
            hoomd.run(1)
...
class updater_reverse_perturbation_swap(unittest.TestCase):
    def setUp(self):
        snap = hoomd.data.make_snapshot(N=5, box=hoomd.data.boxdim(L=20), particle_types=['A','S'])
        ...
        self.s = hoomd.init.read_snapshot(snap)
        self.group0 = hoomd.group.type(type='A')
        self.group1 = hoomd.group.type(type='S')
    ...
    def test_simple_swap(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=1.0,Nswap=1,period=1,phase=0,target_momentum=1)
        hoomd.run(1)
    ...
    def test_swap_momentum_not_velocity(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=2.0,Nswap=1,period=1,target_momentum=10)
        hoomd.run(1)
    ...
    def test_empty_top_slab(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=1.0,Nswap=10,period=1,target_momentum=1)
        hoomd.run(1)
    ...
    def test_simple_swap_outside_slab(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=0.2,Nswap=100,period=1,target_momentum=1)
        hoomd.run(1)
        self.assertAlmostEqual(self.s.particles[0].velocity[0],0.1)
        self.assertAlmostEqual(self.s.particles[1].velocity[0],0.8)
        self.assertAlmostEqual(self.s.particles[2].velocity[0],-0.1)
    ...
    def test_simple_swap_all(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=1.0,Nswap=100,period=1,target_momentum=1)
        hoomd.run(1)
        self.assertAlmostEqual(self.s.particles[1].velocity[0],-0.5)
        self.assertAlmostEqual(self.s.particles[3].velocity[0],0.8)
        self.assertAlmostEqual(self.s.particles[0].velocity[0],-0.1)
    ...
    def test_target_momentum_swap(self):
        ...
        field = azplugins.flow.reverse_perturbation(hoomd.group.all(),width=1.0,Nswap=1,period=1,target_momentum=0.1)
        hoomd.run(1)
        self.assertAlmostEqual(self.s.particles[1].velocity[0],0.8)
        self.assertAlmostEqual(self.s.particles[3].velocity[0],-0.5)
        self.assertAlmostEqual(self.s.particles[0].velocity[0],-0.1)
